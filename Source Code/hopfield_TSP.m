...
while masterCounter <= 10
    ...
    while niter > 0
        ...
        temp = sum(V);
        sumV = sum(temp)
        [validity, tripcost] = compute_trip_cost(V, intercity_distances)
        %  input  number of iterations-- quit for negative
        %niter = input('enter number of iterations (<=0 to quit)')
        if validity == true
            Name = ['CXQ41 Plot', num2str(masterCounter), '.jpg'];
            bar3(V);
            titleName = ['CXQ41 A=100 Final Result: #', num2str(masterCounter), ' Iteration Cost: ', num2str(niter), ' Trip Cost: ', num2str(tripcost)]
            title(titleName)
            print(Name, '-djpeg')
            costRecorder = [costRecorder, tripcost];
    ...
    iterationRecorder = [iterationRecorder, niter]
end